...
// פונקציה להוספת מסמך
export const addDocument = onRequest(async (req, res) => {
  // עטיפת כל הלוגיקה ב-Promise
  return new Promise((resolve) => {
    corsHandler(req, res, async () => {
      try {
        ...
        if (userData.job_title === "מטפל") {
          res.status(403).json({
            success: false,
            message: "אין לך הרשאה להוסיף מסמכים",
          });
          return resolve();
        }
        ...
        try {
          // לולאת for רגילה כדי לאפשר שימוש ב-await
          for (const doc of querySnapshot1.docs) {
            if (doc.data().student_id === newDoc.student_id) {
              // מיזוג המסמך החדש למערך הקיים
              const updatedDocuments = [
                ...(doc.data().documents || []), // מערך קיים (אם קיים)
                { URL: newDoc.URL, description: newDoc.description }, // מסמך חדש
              ];

              // עדכון השדה documents במסמך
          ...
          // אם לא נמצא מסמך מתאים
          res.status(404).json({
            success: false,
            message: "לא נמצא תלמיד עם student_id זה",
          });
        } catch (error) {
          res.status(500).json({
            success: false,
